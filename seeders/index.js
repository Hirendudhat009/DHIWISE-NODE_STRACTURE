...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Norval77' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'NTfkuDK_nGlqplf',
        'username':'Norval77',
        'isDeleted':false,
        'isActive':true,
      ...
      await dbService.updateByQuery(model.user, { 'username':'Norval77' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Nella43' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'TB3tIFYWq_CZPWM',
        'username':'Nella43',
        'isDeleted':false,
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.updateByQuery(model.user, { 'username':'Nella43' }, userToBeInserted);
    }
